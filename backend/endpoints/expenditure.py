...
async def post_income(request: Request):
    ...
    try:
        today = date.today()
        query = f"INSERT INTO {EXPENDITURE_TABLE}(user_id, exp_date, amount, reason) VALUES (%s, %s, %s, %s)"
        cursor = conn.cursor()
        ...
        print(e)
        return {"code": 400, "message": f"Error Creating Expenditure: {e}"}
...
async def get_income_details(user_id: int, page: int = 1):
    ...
    try:
        cursor = conn.cursor()
        data_query = f"SELECT * FROM {EXPENDITURE_TABLE} WHERE user_id = %s ORDER BY exp_date DESC OFFSET %s LIMIT %s"
        cursor.execute(data_query, (user_id, offset, limit))
        res = cursor.fetchall()

        count_query = f"SELECT COUNT(*) FROM {EXPENDITURE_TABLE} WHERE user_id = %s"
...
async def delete_entry(request: Request):
    ...
    try:
        cursor = conn.cursor()
        query = f"DELETE FROM {EXPENDITURE_TABLE} WHERE id = %s AND user_id = %s"
...
def get_total_salary(user_id: int):
    ...
    try:
        cursor = conn.cursor()
        query = f"SELECT SUM(amount) FROM {EXPENDITURE_TABLE} WHERE user_id = %s"
...
def get_this_month_salary(user_id: int):
    ...
    try:
        cursor = conn.cursor()
        query = f"SELECT SUM(amount) FROM {EXPENDITURE_TABLE} WHERE EXTRACT(MONTH FROM exp_date) = %s AND EXTRACT(YEAR FROM exp_date) = %s AND user_id = %s"
...
def get_each_month_expenditure(user_id: int, year: int = 2026):
    ...
    try:
        ...
        query = f"""
        SELECT
            TO_CHAR(exp_date, 'FMMonth') AS month,
            SUM(amount) AS amount,
            EXTRACT(MONTH FROM exp_date) AS month_num
        FROM {EXPENDITURE_TABLE}
        WHERE EXTRACT(YEAR FROM exp_date) = %s AND user_id = %s
...
def get_each_day_expenditure(user_id: int, month: int, year: int):
    ...
    try:
        ...
        query = f"""
        SELECT
            EXTRACT(DAY FROM exp_date) AS day,
            SUM(amount) AS amount
        FROM {EXPENDITURE_TABLE}
        WHERE EXTRACT(YEAR FROM exp_date) = %s AND EXTRACT(MONTH FROM exp_date) = %s AND user_id = %s
        GROUP BY day
        ORDER BY day
        """
